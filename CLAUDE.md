...
**Content:** Spanish-language real estate landing page. Sections: hero, razones, amenidades (carousel + grid), departamentos (tabbed floor plans), acabados (tabbed finishes), about-us, ubicación, contacto, footer.

## Conventions

- No `enum`s, `namespace`s, or `constructor parameter properties`.
- Use `import type` for type-only imports.
- All static assets in `public/` (especially `public/src/` with images), not `src/assets/`.
